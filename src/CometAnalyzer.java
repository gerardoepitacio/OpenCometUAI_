...
/**
 * CometAnalyzer.java Created in 2012 by Benjamin Gyori National University of
 * Singapore e-mail: bgyori@nus.edu.sg
 *
 * When you use this plugin for your work, please cite Gyori BM, Venkatachalam
 * G, et al. OpenComet: An automated tool for comet assay image analysis
 *
 * CometAnalyzer.java is the main internal class for comet analysis.
 * cometAnalyzerRun takes an image as input. It analyzes the image and extracts
 * comets. The comet head and tail are segmented and measurements are
 * calculated. The resulting Comet objects are returned.
 *
...
//        }
        ...
        for (int i = 0; i < Comets.size(); i++) {
            Comet comet = Comets.get(i);
            // Skip invalid cometss
            if (comet.status == Comet.INVALID) {
                continue;
            }
            // Assign an ID to each not-invalid comet
            comet.id = idxValid + 1;
            ...
            // Make polygon ROI
            comet.cometRoi = new PolygonRoi(comet.cometRoi.getConvexHull(), Roi.POLYGON);
            ...
            comet.tailIntensity = (comet.tailArea > 0) ? (comet.tailDNA / comet.tailArea) : 0;
            comet.tailDNApercent = comet.tailDNA / comet.cometDNA; // Multiply by 100 later
            comet.tailMoment = comet.tailLength * comet.tailDNApercent;
            Roi roiTail = (new ShapeRoi(comet.cometRoi)).not(new ShapeRoi(comet.headRoi));
            ...
            comet.tailOliveMoment = comet.tailDNApercent * Math.abs(comet.tailCentroid - comet.headCentroid);
...
//					|| (cometOptions & HEADFIND_PROFILE)!=0){
    ...
    private int getXIntensityCentroid(ImageProcessor ip) {
        ...
        for (x = br.x; x < (br.x + br.width); x++) {
            ...
            if (sumIntensity > fullIntensityHalf) {
                break;
            }
        }
    ...
    private class CometParticleAnalyzer extends ParticleAnalyzer {
        ...
        public CometParticleAnalyzer(int options, int measurements, Object dummy, double minSize, double maxSize, double minCirc, double maxCirc) {
            super(options, measurements, null, minSize, maxSize, minCirc, maxCirc);
